...
export const crud = {
    ...
    create: (entity, jsonData, type, next) => async (dispatch) => {
        dispatch({
            type: actionTypes.REQUEST_LOADING,
            keyState: "create",
            payload: null,
        });
        let data = await request.create(entity, jsonData, type);
        if (next) next(data)
        if (data.success === true) {
            dispatch({
                type: actionTypes.REQUEST_SUCCESS,
    ...
    update: (entity, itemId, jsonData, type, next) => async (dispatch) => {
        dispatch({
            type: actionTypes.REQUEST_LOADING,
            keyState: "update",
            payload: null,
        ...
        let data = await request.update(entity, itemId, jsonData, type);
        if (next) next(data)
        if (data.success === true) {
            dispatch({
                type: actionTypes.REQUEST_SUCCESS,
    ...
    delete: (entity, itemId, next) => async (dispatch) => {
        dispatch({
            type: actionTypes.REQUEST_LOADING,
            keyState: "delete",
            payload: null,
    ...
    banUser: (itemId, ban, next) => async (dispatch) => {
        let data = await request.banUser(itemId, { ban });
        next(data)
    },
